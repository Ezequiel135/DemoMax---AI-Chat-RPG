             ...
             @if (auth.canViewSensitiveKeys()) {
                ...
                <div class="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl relative overflow-hidden">
                   ...
                   <div class="relative z-10 flex items-start gap-3">
                      ...
                      <div>
                         <h4 class="text-yellow-400 font-bold text-sm uppercase tracking-wider mb-1">Safety Warning</h4>
                         <p class="text-xs text-yellow-200/80 leading-relaxed blink-text">
                            Use a <strong>RANDOM KEY</strong> (Chave Aleatória) for your own safety! Never share personal data like CPF or Phone in public keys.
                         </p>
                      </div>
                   </div>
                </div>
                ...
                <div class="space-y-4">
                   <p class="text-sm text-slate-300">
                      Send a contribution directly to this creator via PIX. 
                      <span class="text-slate-500 block text-xs mt-1">Note: This transaction is external. The app does not process payments.</span>
                   </p>
                   ...
                   <p class="text-[10px] text-center text-slate-600 mt-2">
                      Key provided by creator. Verify details in your banking app before confirming.
                   </p>
                </div>
                ...
                <div class="text-center py-6 space-y-4">
                   ...
                   <h4 class="text-white font-bold">Access Restricted</h4>
                   <p class="text-sm text-slate-400 px-4">
                      To prevent fraud and ensure community safety, direct payment keys are only visible to accounts older than <strong>30 days</strong>.
                   </p>
                   @if (auth.isMaster()) {
                      <p class="text-yellow-500 font-bold text-xs uppercase animate-pulse">Master Bypass Active</p>
                   }
...
export class PixDonateModalComponent {
  ...
  copyKey() {
    navigator.clipboard.writeText(this.pixKey()).then(() => {
       ...
       this.toast.show("PIX Key copied to clipboard", "success");
